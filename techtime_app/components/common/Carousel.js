...
const Carousel = ({ children, device }) => {
  ...
  return (
    ...
    >
      <div
        className=" transition-all flex justify-center items-center  space-x-3 whitespace-nowrap w-full"
        style={{ transform: `translateX(-${activeIndex * 1}px)` }}
      >
          ...
          {result.map((child, index) => {
            return (
              <button
                className={`${
                  index === activeIndex
                    ? "bg-techtimeBlue transition-all m-1 h-1.5 w-1.5 rounded-md"
